 ...
 */
;(function(exports) {
  exports.RandomGen = function(seed_name) {
    var buffer = ''
    ...
    function updateLocalStorage() {
      if ( that.ready() ) {
        local_storage[seed_name] = outputOracle(buffer);
     ...
     */
    this.addEntropy = function(entropy, amount) {
      amount = amount || 1;
      events_left -= amount;
      buffer = seedOracle(buffer + entropy);

      this.trigger('entropy++', amount);
    };
    ...
    this.trigger = function(event) {
      ...
      while (len--) {
        cbs[len].apply(null, args);
      };
    };

    // Main
    this.bind('entropy++', function() {
      if ( ready() && ready_queue ) {
        var len = ready_queue.length;
    ...
    if ( local_storage[seed_name] ) {
      /* we've got a seed from last time, add time to it just in case...*/
      buffer = seedOracle(localStorage[seed_name] + (new Date()).valueOf());
      events_left = TOTAL_EVENTS = 0;
    }
  };

  var random = exports.random = new exports.RandomGen();
